      ...
      {authenticated ? (
        <section className='flex flex-col items-center max-sm:w-full max-sm:h-4/5 sm:h-1/2 sm:w-1/3 border bg-white border-gray-300 p-3 shadow-2xl relative'>
          <p
            ...
            className='absolute right-1.5 text-4xl cursor-pointer'
            onClick={() => toggleLoginDialog(false)}
          >
            &times;
          </p>

          {user?.user?.user_metadata?.display_name && (
            <p>{user?.user?.user_metadata?.display_name}</p>
          )}
          <button
            data-testid='logout-user'
            className='px-5 py-3 text-white cursor-pointer bg-violet-500 hover:bg-violet-600'
            onClick={() => clearOutUser()}
          >
            Logout
          </button>
        </section>
        ...
        <form
          ...
          action={formAction}
          className='flex flex-col items-center h-1/2 sm:w-1/3 max-sm:w-full border bg-white border-gray-300 p-3 shadow-2xl relative'
        >
          <span
            title='close login dialog'
            className='absolute right-1.5 text-4xl cursor-pointer'
          ...
          <input
            ...
            minLength={9}
            pattern='^\S+@\S+\.\S+$'
            className='py-2 px-3 w-full my-3 border border-gray-300'
          />
          <input
            type='password'
          ...
          <button
            type='submit'
            className='p-3 w-1/4 bg-violet-500 hover:bg-violet-700 hover:cursor-pointer text-white'
          >
            Login
          </button>
          <p className=' pt-4'>
            Not registered!{' '}
            <Link href='/signup' className='cursor-pointer text-violet-500'>
              {' '}
              Signup here
            </Link>
          </p>
