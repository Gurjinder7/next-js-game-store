...
const Header = () => {
    ...
    const {products,user, toggleLoginDialog, loginDialog, authenticated,clearData, sidebar, toggleSidebar} = useAppStore()
    const router = useRouter()
        ...
        <>
            <nav className="p-3 navbar navbar-expand-lg navbar-dark bg-dark h-[8vh] flex justify-between bg-violet-600 text-white">
                ...
                <div className="max-sm:block sm:hidden">
                    <img src="/ham.svg" className="w-[3rem]" alt="ham-icon" onClick={() => toggleSidebar(true)} />
                </div>
            ...
            </nav>
            {
                ...
                <div className="h-screen w-screen sm:hidden bg-black fixed top-0 left-0 z-20 text-white">
                    <span className="absolute right-2 top-2 text-5xl" onClick={() => toggleSidebar(false)}>&times;</span>
                    <ul className="mt-[15vh]" onClick={() => toggleSidebar(false)}>
                        <li className="my-[4rem] border-b-2 text-center text-4xl pb-3">
                            <Link href="/" className="flex gap-2 justify-around items-center">Home</Link>
                        </li>
                        { authenticated &&
                            <li className="my-[4rem] border-b-2 text-center text-4xl pb-3">
                        ...
                        <li className="my-[4rem] border-b-2 text-center text-4xl relative pb-3">
                            <span className="">
                        {products.length > 0 && <span className=" flex justify-center text-xl items-center absolute right-2 rounded-full p-2 bg-amber-400 text-black font-bold w-[30px] h-[30px]">{products.length}</span>}
                                <Link href="/cart">
                        My Cart
                    </Link>
                    </span>
